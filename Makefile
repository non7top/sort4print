...
.PHONY: help image test check build run lint fmt pack-cities pack-flags shell stop destroy
...
help:
	...
	@echo 'make pack-flags   regenerate assets/flags.bin from ./data'
...
# Compresses the flag SVGs and packs them into the blob the exe carries. They
# stay as drawings so a flag can be rendered at whatever size a print needs.
pack-flags:
	$(RUN) bash tools/gzip-flags.sh data/country-flags-main/svg data/flags-svgz
	$(RUN) cargo run -p pack-flags -- data/flags-svgz assets/flags.bin
